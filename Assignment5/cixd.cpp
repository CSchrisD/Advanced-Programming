...
void reply_get (accepted_socket& client_sock, cix_header& header) {
   ...
   if (is) {
      ...
      log << "reciving header " << header << endl;
      send_packet (client_sock, &header, sizeof header);
      send_packet (client_sock, buffer, length);
      log << "recived " << length << " bytes" << endl;
   } else {
      log << "get: ifsream failed: " << strerror (errno) << endl;
...
void reply_put (accepted_socket& client_sock, cix_header& header) {
   ofstream is; 
   is.open(header.filename);
   if (is.is_open()) {
      //is.seekg (0, is.end);
      int length = header.nbytes;
      //is.seekg (0, is.beg);
      char * buffer = new char [length];
      is << buffer;
      header.command = CIX_ACK;
      ...
      log << "put: ifsream failed: " << strerror (errno) << endl;
      header.command = CIX_NAK;
      header.nbytes = errno;
      send_packet (client_sock, &header, sizeof header);
   }
...
void reply_rm (accepted_socket& client_sock, cix_header& header) {
   ...
   header.command = CIX_ACK;
   send_packet (client_sock, &header, sizeof header);
   send_packet (client_sock, a.c_str(), 0);
}
